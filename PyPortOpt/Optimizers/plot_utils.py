...
def prepossessing_ret(R, name)
 ...
 def cumret_log_plot(df_sub):
    figure(figsize=(15, 6), dpi=80)
    plt.plot(df_sub.index ,np.log(np.cumprod(1 + df_sub["strategy"]/100)), label='strategy')
 ...
 def rolling_vol_plt(df_sub, window_size = 5, rebalance_time = 1):
    figure(figsize=(15, 6), dpi=80)
    plt.plot(rollingwindow_stat(df_sub).index ,rollingwindow_stat(df_sub), label='strategy')
    plt.title('rolling volatility')  
    plt.legend(loc='best')

 def rolling_shar_plt(df_sub, window_size = 5, rebalance_time = 1):
    figure(figsize=(15, 6), dpi=80)
    plt.plot(rollingwindow_shar(df_sub).index ,rollingwindow_shar(df_sub), label='strategy')
    plt.title('rolling sharpe ratio')  
    plt.legend(loc='best')

 def rolling_sortino_plt(df_sub, window_size = 5, rebalance_time = 1):
    figure(figsize=(15, 6), dpi=80)
    plt.plot(rollingwindow_sortino(df_sub).index ,rollingwindow_sortino(df_sub), label='strategy')
