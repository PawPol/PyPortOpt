...
def sigMatShrinkage(sigMat, lambda_l2):
    ...
    if 1 == 1:
        ...
        t = np.dot(np.mean(sig)*np.eye(d), np.eye(d) +
                   (np.ones(d, d)-np.eye(d)) * np.mean(corrs))
...
def minimumVariancePortfolio(sigMat, longShort, maxAlloc=1, lambda_l1=0, lambda_l2=0, assetsOrder=None):
    """
    ...
    if longShort == 0:
        ...
        sigMat3d = sigMat3d + \
            np.diag(np.hstack([-0.1*e_min*np.ones(d), 0.1*e_min*np.ones(2*d)]))
...
def meanVariancePortfolioReturnsTarget(meanVec, sigMat, retTarget, longShort, maxAlloc=1, lambda_l1=0, lambda_l2=0, assetsOrder=None):
    """
